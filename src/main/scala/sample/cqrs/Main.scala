...
//      ActorSystem[Nothing](Guardian(), "Shopping", config(port, httpPort))
      val system =
      ActorSystem[Nothing](Guardian(), "ApplicationIcon", config(port, httpPort))
...
object Guardian {
  ...
  def createProjectionFor(
      ...
      index: Int): AtLeastOnceProjection[Offset, EventEnvelope[ApplicationIcon.Event]] = {
...
//      handler = () => new ShoppingCartProjectionHandler(tag, system))
        val sourceProvider = EventSourcedProvider.eventsByTag[ApplicationIcon.Event](
          system = system,
          readJournalPluginId = CassandraReadJournal.Identifier,
          tag = tag)
        CassandraProjection.atLeastOnce(
          projectionId = ProjectionId("application-icons", tag),
          sourceProvider,
          handler = () => new ApplicationIconProjectionHandler(tag, system))
  }
...
//      val routes = new ShoppingCartRoutes()(context.system) ++ new ApplicationIconRoutes()(context.system)
      val routes = new ApplicationIconRoutes()(context.system)
      new ApplicationIconServer(routes.icon, httpPort)(context.system).start()
